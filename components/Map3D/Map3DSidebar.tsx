...
import { Play, Pause, Search, SlidersHorizontal, TrendingUp, Activity, Database, Zap, Eye, EyeOff } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              {mode === 'cinematic' ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
              Cinematic
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <label className="flex items-center justify-between p-2 bg-bg-hover/50 rounded-lg cursor-pointer hover:bg-bg-hover transition-colors">
              <span className="text-xs text-text-primary">Height = Health</span>
              <input
                type="checkbox"
                checked={visualSettings.showHeight}
                onChange={(e) => onVisualSettingsChange({ ...visualSettings, showHeight: e.target.checked })}
                className="w-4 h-4 rounded border-border-app text-blue-500 focus:ring-2 focus:ring-blue-500"
              />
            </label>
            
            <label className="flex items-center justify-between p-2 bg-bg-hover/50 rounded-lg cursor-pointer hover:bg-bg-hover transition-colors">
              <span className="text-xs text-text-primary">Node Glow</span>
              <input
                type="checkbox"
                checked={visualSettings.showGlow}
                onChange={(e) => onVisualSettingsChange({ ...visualSettings, showGlow: e.target.checked })}
                className="w-4 h-4 rounded border-border-app text-blue-500 focus:ring-2 focus:ring-blue-500"
              />
            </label>
            
            <label className="flex items-center justify-between p-2 bg-bg-hover/50 rounded-lg cursor-pointer hover:bg-bg-hover transition-colors">
              <span className="text-xs text-text-primary">Operator Arcs</span>
              <input
                type="checkbox"
                checked={showArcs}
          ...
          <label className="flex items-center justify-between cursor-pointer">
            ...
            <button
              ...
              className={`relative w-11 h-6 rounded-full transition-colors ${
                filter.activeOnly ? 'bg-blue-500' : 'bg-bg-hover'
              }`}
            >
              <span
